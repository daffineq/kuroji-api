import { Injectable } from '@nestjs/common';
import {
  AnilistAiringSchedule,
  Prisma,
  TmdbReleaseSeason,
  TmdbSeasonEpisode,
} from '@prisma/client';
import {
  TmdbWithRelations,
  TmdbSeasonWithRelations,
  TmdbSeasonEpisodeImagesWithRelations,
  BasicTmdb,
} from '../types/types';
import { ExpectAnime, findBestMatch } from '../../../mapper/mapper.helper';
import { AnilistWithRelations } from '../../anilist/types/types';

@Injectable()
export class TmdbHelper {
  getTmdbData(tmdb: TmdbWithRelations): Prisma.TmdbCreateInput {
    return {
      id: tmdb.id,
      adult: tmdb.adult ?? false,
      backdrop_path: tmdb.backdrop_path ?? undefined,
      episode_run_time: tmdb.episode_run_time ?? [],
      media_type: tmdb.media_type ?? undefined,
      first_air_date: tmdb.first_air_date ?? undefined,
      homepage: tmdb.homepage ?? undefined,
      in_production: tmdb.in_production ?? undefined,
      last_air_date: tmdb.last_air_date ?? undefined,
      name: tmdb.name ?? undefined,
      number_of_episodes: tmdb.number_of_episodes ?? undefined,
      number_of_seasons: tmdb.number_of_seasons ?? undefined,
      original_language: tmdb.original_language ?? undefined,
      original_name: tmdb.original_name ?? undefined,
      origin_country: tmdb.origin_country ?? [],
      overview: tmdb.overview ?? undefined,
      popularity: tmdb.popularity ?? undefined,
      poster_path: tmdb.poster_path ?? undefined,
      tagline: tmdb.tagline ?? undefined,
      status: tmdb.status ?? undefined,
      type: tmdb.type ?? undefined,
      vote_average: tmdb.vote_average ?? undefined,
      vote_count: tmdb.vote_count ?? undefined,

      // Add next episode to air
      next_episode_to_air: tmdb.next_episode_to_air
        ? {
            connectOrCreate: {
              where: { id: tmdb.next_episode_to_air.id },
              create: {
                id: tmdb.next_episode_to_air.id,
                name: tmdb.next_episode_to_air.name ?? undefined,
                overview: tmdb.next_episode_to_air.overview ?? undefined,
                vote_average:
                  tmdb.next_episode_to_air.vote_average ?? undefined,
                vote_count: tmdb.next_episode_to_air.vote_count ?? undefined,
                air_date: tmdb.next_episode_to_air.air_date ?? undefined,
                episode_number:
                  tmdb.next_episode_to_air.episode_number ?? undefined,
                episode_type:
                  tmdb.next_episode_to_air.episode_type ?? undefined,
                production_code:
                  tmdb.next_episode_to_air.production_code ?? undefined,
                runtime: tmdb.next_episode_to_air.runtime ?? undefined,
                season_number:
                  tmdb.next_episode_to_air.season_number ?? undefined,
                still_path: tmdb.next_episode_to_air.still_path ?? undefined,
              },
            },
          }
        : undefined,

      // Add last episode to air
      last_episode_to_air: tmdb.last_episode_to_air
        ? {
            connectOrCreate: {
              where: { id: tmdb.last_episode_to_air.id },
              create: {
                id: tmdb.last_episode_to_air.id,
                name: tmdb.last_episode_to_air.name ?? undefined,
                overview: tmdb.last_episode_to_air.overview ?? undefined,
                vote_average:
                  tmdb.last_episode_to_air.vote_average ?? undefined,
                vote_count: tmdb.last_episode_to_air.vote_count ?? undefined,
                air_date: tmdb.last_episode_to_air.air_date ?? undefined,
                episode_number:
                  tmdb.last_episode_to_air.episode_number ?? undefined,
                episode_type:
                  tmdb.last_episode_to_air.episode_type ?? undefined,
                production_code:
                  tmdb.last_episode_to_air.production_code ?? undefined,
                runtime: tmdb.last_episode_to_air.runtime ?? undefined,
                season_number:
                  tmdb.last_episode_to_air.season_number ?? undefined,
                still_path: tmdb.last_episode_to_air.still_path ?? undefined,
              },
            },
          }
        : undefined,

      // Update seasons relation
      seasons: {
        connectOrCreate:
          tmdb.seasons?.map((s) => ({
            where: { id: s.id },
            create: {
              id: s.id,
              air_date: s.air_date ?? undefined,
              episode_count: s.episode_count ?? undefined,
              name: s.name ?? undefined,
              overview: s.overview ?? undefined,
              poster_path: s.poster_path ?? undefined,
              season_number: s.season_number ?? undefined,
              vote_average: s.vote_average ?? undefined,
            },
          })) ?? [],
      },
    };
  }

  getTmdbSeasonData(
    tmdb: TmdbSeasonWithRelations,
  ): Prisma.TmdbSeasonCreateInput {
    return {
      id: tmdb.id,
      air_date: tmdb.air_date ?? undefined,
      name: tmdb.name ?? undefined,
      overview: tmdb.overview ?? undefined,
      poster_path: tmdb.poster_path ?? undefined,
      season_number: tmdb.season_number, // Required field
      vote_average: tmdb.vote_average ?? undefined,
      episodes: {
        connectOrCreate:
          tmdb.episodes?.map((e) => ({
            where: { id: e.id },
            create: {
              id: e.id,
              air_date: e.air_date ?? undefined,
              episode_number: e.episode_number,
              episode_type: e.episode_type ?? undefined,
              name: e.name ?? undefined,
              overview: e.overview ?? undefined,
              production_code: e.production_code ?? undefined,
              runtime: e.runtime ?? undefined,
              season_number: e.season_number,
              still_path: e.still_path ?? undefined,
              vote_average: e.vote_average ?? undefined,
              vote_count: e.vote_count ?? undefined,
              show: {
                connect: {
                  id: e.show_id,
                },
              },
            },
          })) ?? [],
      },
      show: {
        connect: {
          id: tmdb.show_id,
        },
      },
    };
  }

  getTmdbEpisodeImagesData(
    tmdb: TmdbSeasonEpisodeImagesWithRelations,
  ): Prisma.TmdbSeasonEpisodeImagesCreateInput {
    return {
      id: tmdb.id,
      stills: {
        create: tmdb.stills.map((s) => ({
          aspect_ratio: s.aspect_ratio ?? null,
          height: s.height ?? null,
          width: s.width ?? null,
          iso_639_1: s.iso_639_1 ?? null,
          file_path: s.file_path ?? null,
          vote_average: s.vote_average ?? null,
          vote_count: s.vote_count ?? null,
        })),
      },
      episode: {
        connect: {
          id: tmdb.episodeId,
        },
      },
    };
  }
}

const ALLOWED_COUNTRIES = ['JP', 'KR', 'CN'];
const ALLOWED_LANGUAGES = ['ja', 'ko', 'zh'];

const isProbablyAnime = (tmdb: BasicTmdb): boolean => {
  const countryMatch =
    tmdb.origin_country?.some((c) => ALLOWED_COUNTRIES.includes(c)) ?? false;

  const languageMatch = ALLOWED_LANGUAGES.includes(tmdb.original_language);

  return countryMatch || languageMatch;
};

function tmdbToBasic(tmdb: TmdbWithRelations): BasicTmdb {
  return {
    id: tmdb.id,
    original_name: tmdb.original_name ?? '',
    media_type: tmdb.media_type ?? '',
    name: tmdb.name ?? '',
    first_air_date: tmdb.first_air_date ?? '',
    original_language: tmdb.original_language ?? '',
    origin_country: tmdb.origin_country,
  };
}

export function findBestMatchFromCandidates(
  anilist: AnilistWithRelations,
  candidates: TmdbWithRelations[],
): TmdbWithRelations | null {
  const searchAnime: ExpectAnime = {
    title: {
      romaji: (anilist.title as { romaji: string }).romaji,
      english: (anilist.title as { english: string }).english,
      native: (anilist.title as { native: string }).native,
    },
    year: anilist.startDate?.year ?? undefined,
    episodes: anilist.episodes ?? undefined,
  };

  const mediaTypeFiltered = candidates.filter((tmdb) =>
    isProbablyAnime(tmdbToBasic(tmdb)),
  );

  const bestMatch = findBestMatch(searchAnime, mediaTypeFiltered);
  if (bestMatch) {
    return bestMatch.result;
  }

  return null;
}

export function findBestMatchFromSearch(
  anilist: AnilistWithRelations,
  results: BasicTmdb[] | undefined,
): BasicTmdb | null {
  if (!results || !Array.isArray(results)) return null;

  const searchAnime: ExpectAnime = {
    title: {
      romaji: (anilist.title as { romaji: string }).romaji,
      english: (anilist.title as { english: string }).english,
      native: (anilist.title as { native: string }).native,
    },
    year: anilist.startDate?.year ?? undefined,
    episodes: anilist.episodes ?? undefined,
  };

  const resultsFiltered = results.filter((tmdb) => isProbablyAnime(tmdb));

  const bestMatch = findBestMatch(
    searchAnime,
    resultsFiltered.map((result) => ({
      id: result.id,
      title: {
        english: result.name,
        native: result.original_name,
      },
    })),
  );

  if (bestMatch) {
    return results.find((r) => r.id === bestMatch.result.id) || null;
  }

  return null;
}

export function filterSeasonEpisodes(
  seasonData: TmdbSeasonWithRelations,
): TmdbSeasonWithRelations {
  const filteredEpisodes = seasonData.episodes.map((episode) => ({
    id: episode.id,
    air_date: episode.air_date,
    episode_number: episode.episode_number,
    episode_type: episode.episode_type,
    name: episode.name,
    overview: episode.overview,
    production_code: episode.production_code,
    runtime: episode.runtime,
    season_number: episode.season_number,
    show_id: episode.show_id,
    still_path: episode.still_path,
    vote_average: episode.vote_average,
    vote_count: episode.vote_count,
  })) as TmdbSeasonEpisode[];

  seasonData.episodes = filteredEpisodes;
  return seasonData;
}

export function findBestMatchingSeason(
  tmdbSeasons: TmdbReleaseSeason[],
  anilist: AnilistWithRelations,
  anilistStartDate: string | null,
  anilistEndDate: string | null,
  firstAiringDate: string | null,
  lastAiringDate: string | null,
  expectedEpisodeCount: number,
): any | null {
  const candidates: Array<{ season: any; score: number; reasons: string[] }> =
    [];

  for (const season of tmdbSeasons) {
    let score = 0;
    const reasons: string[] = [];

    // Skip season 0 (specials) unless it's the only option
    if (season.season_number === 0 && tmdbSeasons.length > 1) {
      continue;
    }

    // Strategy 1: Exact date matching
    if (anilistStartDate && season.air_date === anilistStartDate) {
      score += 100;
      reasons.push('exact_start_date_match');
    }

    // Strategy 2: Airing schedule date matching
    if (firstAiringDate && season.air_date === firstAiringDate) {
      score += 90;
      reasons.push('airing_schedule_start_match');
    }

    // Strategy 3: Close date matching (within 7 days)
    if (anilistStartDate && season.air_date) {
      const daysDiff = Math.abs(
        (new Date(anilistStartDate).getTime() -
          new Date(season.air_date).getTime()) /
          (1000 * 60 * 60 * 24),
      );
      if (daysDiff <= 7) {
        score += Math.max(80 - daysDiff * 10, 10);
        reasons.push(`close_date_match_${Math.round(daysDiff)}_days`);
      }
    }

    // Strategy 4: Episode count matching
    if (expectedEpisodeCount > 0 && season.episode_count) {
      const episodeCountDiff = Math.abs(
        season.episode_count - expectedEpisodeCount,
      );
      if (episodeCountDiff === 0) {
        score += 50;
        reasons.push('exact_episode_count');
      } else if (episodeCountDiff <= 2) {
        score += 30 - episodeCountDiff * 10;
        reasons.push(`close_episode_count_diff_${episodeCountDiff}`);
      }
    }

    // Strategy 5: Season name matching with AniList titles
    if (season.name && anilist.title) {
      const seasonNameLower = season.name.toLowerCase();
      const titles = [
        anilist.title.romaji,
        anilist.title.english,
        anilist.title.native,
        ...(anilist.synonyms || []),
      ].filter(Boolean);

      for (const title of titles) {
        if (title && seasonNameLower.includes(title.toLowerCase())) {
          score += 30;
          reasons.push('season_name_title_match');
          break;
        }
      }
    }

    // Strategy 6: Year matching
    if (anilist.seasonYear && season.air_date) {
      const seasonYear = new Date(season.air_date).getFullYear();
      if (seasonYear === anilist.seasonYear) {
        score += 25;
        reasons.push('year_match');
      } else if (Math.abs(seasonYear - anilist.seasonYear) <= 1) {
        score += 10;
        reasons.push('close_year_match');
      }
    }

    // Strategy 7: Season number preference (prefer season 1 if similar scores)
    if (season.season_number === 1) {
      score += 5;
      reasons.push('season_1_preference');
    }

    // Strategy 8: Penalty for seasons with no air date
    if (!season.air_date) {
      score -= 20;
      reasons.push('no_air_date_penalty');
    }

    if (score > 0) {
      candidates.push({ season, score, reasons });
    }
  }

  // Sort by score descending
  candidates.sort((a, b) => b.score - a.score);

  return candidates.length > 0 ? candidates[0].season : tmdbSeasons[0];
}

export function findMatchingEpisodes(
  episodes: TmdbSeasonEpisode[],
  airingSchedule: AnilistAiringSchedule[],
  anilistStartDate: string | null,
  anilistEndDate: string | null,
  firstAiringDate: string | null,
  lastAiringDate: string | null,
  today: string,
): any[] {
  const matchingEpisodes: any[] = [];
  let foundStart = false;

  const airingDateMap = new Map();
  for (const schedule of airingSchedule) {
    const airDate = new Date((schedule.airingAt ?? 0) * 1000)
      .toISOString()
      .split('T')[0];
    airingDateMap.set(schedule.episode, airDate);
  }

  for (let i = 0; i < episodes.length; i++) {
    const ep = episodes[i];

    // Skip episodes with no air date or future episodes
    if (!ep.air_date || ep.air_date > today) {
      continue;
    }

    // Find start point using multiple strategies
    if (!foundStart) {
      let shouldStart = false;

      // Strategy 1: Exact start date match
      if (anilistStartDate && ep.air_date === anilistStartDate) {
        shouldStart = true;
      }
      // Strategy 2: First airing date match
      else if (firstAiringDate && ep.air_date === firstAiringDate) {
        shouldStart = true;
      }
      // Strategy 3: Airing schedule episode match
      else if (
        airingDateMap.has(ep.episode_number) &&
        airingDateMap.get(ep.episode_number) === ep.air_date
      ) {
        shouldStart = true;
      }
      // Strategy 4: First episode if no better match found and we're past expected start
      else if (i === 0 && anilistStartDate && ep.air_date >= anilistStartDate) {
        shouldStart = true;
      }

      if (!shouldStart) continue;
      foundStart = true;
    }

    matchingEpisodes.push(ep);

    // Check for end conditions
    if (anilistEndDate && ep.air_date === anilistEndDate) {
      break;
    }
    if (lastAiringDate && ep.air_date === lastAiringDate) {
      break;
    }
  }

  return matchingEpisodes;
}
