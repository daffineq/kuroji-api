...
export const getAnilistPrismaData = async (anime: AnilistMedia): Promise<Prisma.AnilistCreateInput> => {
  const isMalExist = anime.idMal
    ? !!(await prisma.anilist.findUnique({
  ...
  // Get last episode (last in the schedule)
  const lastEpisode = anime?.airingSchedule?.edges
    ? [...anime.airingSchedule.edges].sort((a, b) => (b.node.episode ?? 0) - (a.node.episode ?? 0))[0]?.node
    : undefined;
