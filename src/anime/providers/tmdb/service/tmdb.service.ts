...
export class TmdbService {
  ...
  async getEpisodeDetails(epId: number): Promise<TmdbSeasonEpisodeWithRelations> {
    const include = {
      images: {
        include: {
          stills: {
    ...
    const images = await this.fetchEpisodeImages(existingEpisode.show_id, existingEpisode.season_number, existingEpisode.episode_number);
  ...
  async getEpisodeDetailsByAnilist(id: number, ep: number): Promise<TmdbSeasonEpisodeWithRelations> {
  ...
  async fetchEpisodeImages(tvId: number, seasonNumber: number, episode: number): Promise<TmdbSeasonEpisodeImagesWithRelations> {
    return this.customHttpService.getResponse(TMDB.getEpisodeImages(tvId, seasonNumber, episode));
  }
  ...
  async saveImages(images: TmdbSeasonEpisodeImagesWithRelations): Promise<TmdbSeasonEpisodeImagesWithRelations> {
